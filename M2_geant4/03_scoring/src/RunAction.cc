                          ...
                          G4double primaryPhi)
{
  ...
  const auto& edep = ;//Ricava l'energia totale dall'evento che gli passiamo da argomento

  for (G4int iPlane = 0; iPlane < kNPlanes; ++iPlane) {
    for (G4int iBar = 0; iBar < kNBars; ++iBar) {
      fEdep[iPlane][iBar] = ; //Riempi con variabile e unità corretta
    }
  }

  // Copia delle variabili del primario lette dal G4Event.
  fPrimaryEnergy = ; //Riempi con variabile e unità corretta
  fPrimaryX0     = ; //Riempi con variabile e unità corretta
  fPrimaryY0     = ; //Riempi con variabile e unità corretta
  fPrimaryZ0     = ; //Riempi con variabile e unità corretta
  fPrimaryTheta  = ; //Riempi con variabile e unità corretta
  fPrimaryPhi    = ; //Riempi con variabile e unità corretta
